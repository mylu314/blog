...
## <span id="jump1">SVM<span>
  ...
  * So it'll be nice if we can all achieve a general linear SVM somehow. Then comes the kernel trick.
  * While every dot product is replaced by a nonlinear kernel function(essentially using a new similarity measure), a linear SVM is achieved following the original algorithm(essentially transforming the feature space into a new one).
